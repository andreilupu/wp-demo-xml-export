...
spl_autoload_register('demo_xml_Meta_Box::autoload_helpers');

$meta_boxes = array();
$meta_boxes = apply_filters( 'demo_xml_meta_boxes', $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new demo_xml_Meta_Box( $meta_box );
}

define( 'PIXPROOF_META_BOX_URL', demo_xml_Meta_Box::get_meta_box_url() );

/**
 * Create meta boxes
 */
class demo_xml_Meta_Box {
	...
	protected static $mb_defaults = array(
		...
		'demo_xml_styles' => true, // Include cmb bundled stylesheet
		'fields'     => array(),
	);
	...
	function __construct( $meta_box ) {
		...
		$allow_frontend = apply_filters( 'demo_xml_allow_frontend', true, $meta_box );
		...
		$show_filters = 'demo_xml_Meta_Box_Show_Filters';
		foreach ( get_class_methods( $show_filters ) as $filter ) {
			add_filter( 'demo_xml_show_on', array( $show_filters, $filter ), 10, 2 );
		}
	...
	public function register_scripts() {
		...
		wp_localize_script( 'cmb-scripts2', 'demo_xml_l10', array(
		...
		wp_register_style( 'demo_xml-styles', PIXPROOF_META_BOX_URL . 'style.css', $styles );
	...
	public function do_scripts( $hook ) {
		...
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			...
			if ( get_post_type() === 'proof_gallery' ) {
				...
				if ( $this->_meta_box['demo_xml_styles'] != false )
					wp_enqueue_style( 'demo_xml-styles' );
	...
	public function add_metaboxes() {
		...
		foreach ( $this->_meta_box['pages'] as $page ) {
			if ( apply_filters( 'demo_xml_show_on', true, $this->_meta_box ) )
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		// get box types
		$types = demo_xml_Meta_Box_types::get();

		// Use nonce for verification
		echo "\n<!-- Begin CMB Fields -->\n";
		wp_nonce_field( self::nonce(), 'wp_meta_box_nonce', false, true );
		do_action( 'demo_xml_before_table', $meta_box, $object_id, $object_type );
		echo '<table class="form-table demo_xml_metabox">';
		...
		foreach ( $meta_box['fields'] as $field ) {
			...
			$field['default'] = apply_filters( 'demo_xml_default_filter', $field['default'], $field, $object_id, $object_type );
			// 'demo_xml_std_filter' deprectated, use 'demo_xml_default_filter' instead
			$field['default'] = apply_filters( 'demo_xml_std_filter', $field['default'], $field, $object_id, $object_type );
			...
			// (assuming no one would want to save 'demo_xml_no_override_val' as a value)
			$meta = apply_filters( 'demo_xml_override_meta_value', 'demo_xml_no_override_val', $object_id, $field, $object_type );

			// If no override, get our meta
			if ( $meta === 'demo_xml_no_override_val' )
				$meta = self::get_data();
			...
			echo '<tr class="cmb-type-'. sanitize_html_class( $field['type'] ) .' demo_xml_id_'. sanitize_html_class( $field['id'] ) . $classes .'">';
		...
		echo '</table>';
		do_action( 'demo_xml_after_table', $meta_box, $object_id, $object_type );
		echo "\n<!-- End CMB Fields -->\n";
	...
	public static function save_fields( $meta_box, $object_id, $object_type = '' ) {
		...
		if ( ! apply_filters( 'demo_xml_show_on', true, $meta_box ) )
			return;
		...
		do_action( "demo_xml_save_{$object_type}_fields", $object_id, $meta_box['id'], $updated, $meta_box );
	...
	public static function sanitization_cb( $meta_value, $field = array() ) {
		...
		// Validation via 'demo_xml_Meta_Box_Sanitize' (with fallback filter)
		return call_user_func( array( demo_xml_Meta_Box_Sanitize::get(), $field['type'] ), $meta_value, $field );
	}
	...
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$demo_xml_url = str_replace( DIRECTORY_SEPARATOR, '/', $content_url );

		} else {
		  $demo_xml_url = str_replace(
		...
		return trailingslashit( apply_filters('demo_xml_meta_box_url', $demo_xml_url ) );
	}
	 ...
	 * @uses apply_filters() Calls 'demo_xml_override_option_get_$option_key' hook to allow
	...
	public static function _get_option( $option_key, $default = false ) {
		...
		$test_get = apply_filters( "demo_xml_override_option_get_$option_key", 'demo_xml_no_override_option_get', $default );

		if ( $test_get !== 'demo_xml_no_override_option_get' )
			return $test_get;
	 ...
	 * @uses apply_filters() Calls 'demo_xml_override_option_save_$option_key' hook to allow
	...
	public static function save_option( $option_key ) {
		...
		$test_save = apply_filters( "demo_xml_override_option_save_$option_key", 'demo_xml_no_override_option_save', $to_save );

		if ( $test_save !== 'demo_xml_no_override_option_save' )
			return $test_save;
...
// Handle oembed Ajax
add_action( 'wp_ajax_demo_xml_oembed_handler', array( 'demo_xml_Meta_Box_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_demo_xml_oembed_handler', array( 'demo_xml_Meta_Box_ajax', 'oembed_handler' ) );
 ...
 * @return array               Options array or specific field
 */
function demo_xml_get_option( $option_key, $field_id = '' ) {
	return demo_xml_Meta_Box::get_option( $option_key, $field_id );
}
 ...
 * @param int   $object_id  Object ID
 */
function demo_xml_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		demo_xml_print_metabox( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function demo_xml_print_metabox( $meta_box, $object_id ) {
	$cmb = new demo_xml_Meta_Box( $meta_box );
	if ( $cmb ) {

		demo_xml_Meta_Box::set_object_id( $object_id );
		...
		if ( $meta_box['demo_xml_styles'] != false )
			wp_enqueue_style( 'demo_xml-styles' );

		demo_xml_Meta_Box::show_form( $meta_box );
	}
 ...
 * @param int   $object_id Object ID
 */
function demo_xml_save_metabox_fields( $meta_box, $object_id ) {
	demo_xml_Meta_Box::save_fields( $meta_box, $object_id );
}
 ...
 */
function demo_xml_metabox_form( $meta_box, $object_id, $echo = true ) {

	$meta_box = demo_xml_Meta_Box::set_mb_defaults( $meta_box );
	...
	demo_xml_Meta_Box::set_object_type( demo_xml_Meta_Box::set_mb_type( $meta_box ) );
	...
	if (
		...
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], demo_xml_Meta_Box::nonce() )
		&& $_POST['object_id'] == $object_id
	)
		demo_xml_save_metabox_fields( $meta_box, $object_id );
	...
	$form_format = apply_filters( 'demo_xml_frontend_form_format', '<form class="cmb-form" method="post" id="%s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%s">%s<input type="submit" name="submit-cmb" value="%s" class="button-primary"></form>', $object_id, $meta_box, $form );

	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'demo_xml_l10n' ) );
