...
(function(window, document, $, undefined){
	...
	window.CMB = {
		...
		init: function() {
			CMB.log( window.demo_xml_l10 );

			// hide our spinner gif if we're on a MP6 dashboard
			if ( window.demo_xml_l10.new_admin_style ) {
			 ...
			 */
			$('.demo_xml_timepicker').each( function() {
			 ...
			 */
			$('.demo_xml_datepicker').each( function() {
			...
			$("#ui-datepicker-div").wrap('<div class="demo_xml_element" />');

			/**
			 * Initialize color picker
			 */
			if (typeof jQuery.wp === 'object' && typeof jQuery.wp.wpColorPicker === 'function') {
				$('input:text.demo_xml_colorpicker').wpColorPicker();
			} else {
				$('input:text.demo_xml_colorpicker').each( function(i) {
			...
			$('.demo_xml_metabox')
			.on( 'change', '.demo_xml_upload_file', function() {
				CMB.formfield = $(this).attr('name');
				$('#' + CMB.formfield + '_id').val("");
			})
			.on( 'click', '.demo_xml_upload_button', function(event) {
				...
				var attachment = true;
				var isList = $self.hasClass( 'demo_xml_upload_list' );
				...
				CMB.file_frames[CMB.formfield] = wp.media.frames.file_frame = wp.media({
					...
					button: {
						text: window.demo_xml_l10.upload_file
					},
				...
				CMB.file_frames[CMB.formfield].on( 'select', function() {
					...
					if ( isList ) {
						...
						$( attachment ).each( function() {
							if ( this.type && this.type === 'image' ) {
								...
								uploadStatus = '<li class="img_status">'+
									...
									'<p><a href="#" class="demo_xml_remove_file_button" rel="'+ CMB.formfield +'['+ this.id +']">'+ demo_xml_l10.remove_image +'</a></p>'+
									'<input type="hidden" id="filelist-'+ this.id +'" name="'+ CMB.formfield +'['+ this.id +']" value="' + this.url + '">'+
								'</li>';

							} else {
								// Standard generic output if it's not an image.
								uploadStatus = '<li>'+ demo_xml_l10.file +' <strong>'+ this.filename +'</strong>&nbsp;&nbsp;&nbsp; (<a href="' + this.url + '" target="_blank" rel="external">'+ demo_xml_l10.download +'</a> / <a href="#" class="demo_xml_remove_file_button" rel="'+ CMB.formfield +'['+ this.id +']">'+ demo_xml_l10.remove_file +'</a>)'+
									'<input type="hidden" id="filelist-'+ this.id +'" name="'+ CMB.formfield +'['+ this.id +']" value="' + this.url + '">'+
								'</li>';
						...
						// Append each item from our fileGroup array to .demo_xml_media_status
						$( fileGroup ).each( function() {
							$formfield.siblings('.demo_xml_media_status').slideDown().append(this);
						});
						...
						if ( attachment.type && attachment.type === 'image' ) {
							// image preview
							uploadStatus = '<div class="img_status"><img style="max-width: 350px; width: 100%; height: auto;" src="' + attachment.url + '" alt="'+ attachment.filename +'" title="'+ attachment.filename +'" /><p><a href="#" class="demo_xml_remove_file_button" rel="' + CMB.formfield + '">'+ demo_xml_l10.remove_image +'</a></p></div>';
						} else {
							// Standard generic output if it's not an image.
							uploadStatus = demo_xml_l10.file +' <strong>'+ attachment.filename +'</strong>&nbsp;&nbsp;&nbsp; (<a href="'+ attachment.url +'" target="_blank" rel="external">'+ demo_xml_l10.download +'</a> / <a href="#" class="demo_xml_remove_file_button" rel="'+ CMB.formfield +'">'+ demo_xml_l10.remove_file +'</a>)';
						}

						// add/display our output
						$formfield.siblings('.demo_xml_media_status').slideDown().html(uploadStatus);
					}
				});
			...
			.on( 'click', '.demo_xml_remove_file_button', function(event) {
				var $self        = $(this);
				if ( $self.is( '.attach_list .demo_xml_remove_file_button' ) ){
				...
				if ( ! $container.length ) {
					$self.parents('.demo_xml_media_status').html('');
				} else {
					$container.html('');
				}
				return false;
			...
			.on( 'click', '.add-row-button', function(e) {
				...
				var input = $('input.demo_xml_datepicker',row);
				...
				// row.find('input:text.demo_xml_colorpicker').wpColorPicker();
			...
			// ajax when typing
			.on( 'keyup', '.demo_xml_oembed', function(event) {
				// fire our ajax function
				CMB.doAjax($(this), event);
			});

			// ajax on paste
			$('.demo_xml_oembed').bind( 'paste', function(e) {
				var pasteitem = $(this);
				...
				setTimeout( function() {
					...
					CMB.spinner( '.postbox table.demo_xml_metabox', true );
				}, 2000);
			});
		 ...
		 */
		log: function() {
			if ( window.demo_xml_l10.script_debug && console && typeof console.log === 'function' ) {
		...
		doAjax: function(obj, e) {
			...
			if (e === 'paste' || e.which <= 90 && e.which >= 48 || e.which >= 96 && e.which <= 111 || e.which == 8 || e.which == 9 || e.which == 187 || e.which == 190) {
				...
				var context = obj.parents('.demo_xml_metabox tr td');
				...
				setTimeout( function() {
					...
					if ($('.demo_xml_oembed:focus').val() != oembed_url)
						return;
					$.ajax({
						type : 'post',
						dataType : 'json',
						url : window.demo_xml_l10.ajaxurl,
						data : {
							'action': 'demo_xml_oembed_handler',
							'oembed_url': oembed_url,
							'oembed_width': oembed_width > 300 ? oembed_width : 300,
							'field_id': field_id,
							'object_id': obj.data('objectid'),
							'object_type': obj.data('objecttype'),
							'demo_xml_ajax_nonce': window.demo_xml_l10.ajax_nonce
						},
		...
		resizeoEmbeds: function() {
			$('table.demo_xml_metabox').each( function( index ) {
				var self = $(this);
